...
public class UfoDAO {
	...
	public List<String> getStati(int anno) {
		...
		final String sql = "SELECT  DISTINCT state FROM sighting s1 WHERE Extract(YEAR FROM s1.`datetime`)=? AND country='us'";
		...
		try {
			...
			while (rs.next()) {	
				String s= new String (rs.getString("state"));
				o.add(s);	}	
			st.close();
			conn.close();
			return o;
	...
	public List<Sighting> getAllStati(int anno) {
		...
		try {
			...
			while (rs.next()) {	
				Sighting s= new Sighting (rs.getInt("id"), rs.getDate("datetime").toLocalDate(),rs.getString("state"),
	...
	public static void main(String[] args) {
		...
		List<Sighting> years = dao.getAllStati(1950);
		System.out.println(years);
		List<String> s = dao.getStati(1950);
